...
import { LucideLifeBuoy, LucideFingerprint } from '@lucide/angular';

@Component({
  selector: 'app-auth-layout',
  standalone: true,
  imports: [RouterOutlet, RouterLinkWithHref, LucideLifeBuoy, LucideFingerprint],
...
export class AuthLayoutComponent implements OnDestroy {
  ...
  constructor(@Inject(DOCUMENT) private document: Document) {
    // نهج آمن للتحقق من التخزين المحلي في بيئة المتصفح
    afterNextRender(() => {
    ...
    // استخدام Effect لمراقبة وتطبيق تغييرات الثيم تلقائياً
    effect(() => {
  ...
  private initCanvas() {
    ...
    this.particles = [];
    // تقليل عدد الجزيئات قليلاً لتحسين الأداء على الشاشات الكبيرة
