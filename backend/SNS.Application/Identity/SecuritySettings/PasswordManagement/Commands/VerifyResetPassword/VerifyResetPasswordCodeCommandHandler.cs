...
public sealed class VerifyResetPasswordCodeCommandHandler : ICommandHandler<VerifyResetPasswordCodeCommand, VerifyResetPasswordResponseDto>
{
    ...
    public async Task<Result<VerifyResetPasswordResponseDto>> Handle(VerifyResetPasswordCodeCommand request, CancellationToken cancellationToken)
    {
        ...
        var newToken = _generatorService.GenerateSecureString();
        ...
        return Result<VerifyResetPasswordResponseDto>.Success(new VerifyResetPasswordResponseDto(
            UserId: request.UserId,
            Token: newToken), OperationStatusCode.Success);
    }
}
