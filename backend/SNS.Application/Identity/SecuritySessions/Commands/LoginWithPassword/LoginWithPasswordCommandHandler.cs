...
public sealed class LoginWithPasswordCommandHandler : ICommandHandler<LoginWithPasswordCommand, LoginResponseDto>
{
    ...
    public async Task<Result<LoginResponseDto>> Handle(LoginWithPasswordCommand request, CancellationToken cancellationToken)
    {
        ...
        if (user == null)
        {
            ...
            return Result<LoginResponseDto>.Failure(UserStatusCodes.Deactivated);
        }

        if (user.Status == UserStatus.PermanentlyBanned)
        {
            return Result<LoginResponseDto>.Failure(UserStatusCodes.Banned);
        }
        ...
        if (user.Status == UserStatus.Suspended && user.SuspendedUntil > DateTime.UtcNow)
        {
            return Result<LoginResponseDto>.Failure(
                new LoginResponseDto(SuspendedUntil: user.SuspendedUntil, SuspensionReason: user.SuspensionReason), 
        ...
        try
        {
            ...
            if (!isPasswordValid)
            {
                ...
                return Result<LoginResponseDto>.Failure(UserStatusCodes.FailedLoginAttempt);
            }
    ...
    private async Task<Result<LoginResponseDto>> HandleCriticalRiskLoginAsync(
        User user,
        ...
        CancellationToken cancellationToken)
    {
        ...
        return Result<LoginResponseDto>.Failure(new LoginResponseDto(
            SuspendedUntil: user.SuspendedUntil,
            SuspensionReasonCode: SecurityStatusCodes.CriticalLoginRisk), UserStatusCodes.Suspended);
    }

    private async Task<Result<LoginResponseDto>> HandleHighRiskLoginAsync(
        User user,
        ...
        CancellationToken cancellationToken)
    {
        ...
        // تصعيد التدفق لتوليد تحدي الـ MFA حتماً مع الحفاظ على مطابقة الأنواع (Result<LoginResponseDto>)
        return await HandleMediumRiskLoginAsync(user, recipientAddress, cancellationToken);
    }

    private async Task<Result<LoginResponseDto>> HandleMediumRiskLoginAsync(
        User user,
        string recipientAddress,
        CancellationToken cancellationToken)
    {
        ...
        {
            await _unitOfWork.RollbackTransactionAsync(cancellationToken);
            return Result<LoginResponseDto>.Failure(sendResult.StatusCode);
        }
    ...
    private async Task<Result<LoginResponseDto>> HandleLowRiskLoginAsync(
        User user,
        RequestInformationModel requestInformationModel,
        string recipientAddress,
        CancellationToken cancellationToken)
    {
        ...
        {
            await _unitOfWork.RollbackTransactionAsync(cancellationToken);
            return Result<LoginResponseDto>.Failure(sessionsResult.StatusCode);
        }
        ...
        {
            await _unitOfWork.RollbackTransactionAsync(cancellationToken);
            return Result<LoginResponseDto>.Failure(tokenResult.StatusCode);
        }
