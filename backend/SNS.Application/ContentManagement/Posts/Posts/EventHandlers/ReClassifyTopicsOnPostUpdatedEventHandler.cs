...
public class ReClassifyTopicsOnPostUpdatedEventHandler
    : INotificationHandler<DomainEventNotification<PostUpdatedEvent>>
{
    ...
    private readonly IAppLogger<ReClassifyTopicsOnPostUpdatedEventHandler> _logger;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IFileStorageService _fileStorageService;

    public ReClassifyTopicsOnPostUpdatedEventHandler(
        IApplicationDbContext dbContext,
        ITopicClassifier topicClassifier,
        IPostSearchService postSearchService,
        IRepository<PostTopic> postTopicRepo,
        IAppLogger<ReClassifyTopicsOnPostUpdatedEventHandler> logger,
    ...
    public async Task Handle(DomainEventNotification<PostUpdatedEvent> notification, CancellationToken cancellationToken)
    {
        ...
        if (domainEvent.RequiresReclassification)
        {
            ...
            try
            {
                ...
                await _postTopicRepo.ExecuteDeleteAsync(
                    pt => pt.PostId == post.Id,
                    cancellationToken);
