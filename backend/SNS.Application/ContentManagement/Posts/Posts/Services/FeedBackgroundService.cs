...
public class FeedBackgroundService : IFeedBackgroundService
{
    ...
    public async Task ComputeAndCacheUserFeedAsync(Guid profileId, FeedRequestParameter feedParams)
    {
        ...
        try
        {
            ...
            var excludedPosts = feedParams.ExcludedPostsIds.ToHashSet();
            var excludedProfiles = feedParams.ExcludedProfilesIds.ToHashSet();
            var communities = feedParams.CommunitiesIds.ToHashSet();
            var userTopicsDict = feedParams.Topics.ToDictionary(x => x.TopicId, x => x.Score);
            var userTagsDict = feedParams.Tags.ToDictionary(x => x.TagId, x => x.Score);
            var interestedTopics = userTopicsDict.Keys.ToHashSet();
            ...
            foreach (var post in rawCandidates)
            {
                ...
                double followBonus = followedProfiles.Contains(post.AuthorId) ? 30 : 0;
                ...
                {
                    if (userTagsDict.TryGetValue(postTag.TagId, out double userTagScore))
                    {
                        tagInterestBonus += userTagScore * postTag.Confidence * 10;
                    }
                }
