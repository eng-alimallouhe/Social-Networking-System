...
internal sealed class FeedFallbackService : IFeedFallbackService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (rawPosts.Count < pageSize)
        {
            var remainingCount = pageSize - rawPosts.Count;

            var existingPostIds = rawPosts
                .Select(p => p.Id)
                .ToHashSet();
            ...
            rawPosts.AddRange(additionalPosts);
        }
