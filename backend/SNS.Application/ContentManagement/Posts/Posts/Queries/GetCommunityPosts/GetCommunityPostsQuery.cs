...
namespace SNS.Application.ContentManagement.Posts.Posts.Queries.GetCommunityPosts;

/// <summary>
/// Represents a query to retrieve paginated posts published in a specific community.
/// Enforces community privacy rules: non-members cannot access posts in private communities.
/// </summary>
...
/// <summary>
/// Handles the execution of <see cref="GetCommunityPostsQuery"/> to retrieve community posts.
/// </summary>
internal sealed class GetCommunityPostsQueryHandler : IQueryHandler<GetCommunityPostsQuery, Paged<PostOverviewDto>>
{
    ...
    public async Task<Result<Paged<PostOverviewDto>>> Handle(GetCommunityPostsQuery request, CancellationToken cancellationToken)
    {
        ...
        var baseQuery = _dbContext.Posts
            .AsNoTracking()
            .Where(p => p.CommunityId == request.CommunityId && p.IsActive && (p.Status == PostStatus.Accepted || p.Status == null));
        ...
        var rawPosts = await baseQuery
            ...
            .Select(p => new
            {
                ...
                CurrentUserReaction = p.Reactions.Where(r => r.ReactorId == currentProfileId).Select(r => (ReactionType?)r.Type).FirstOrDefault(),
